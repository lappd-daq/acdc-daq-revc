...
std::vector<uint64_t> Ethernet::RecieveBurst(int numwords, int timeout_sec, int timeout_us)
{
    ...
    while(wordsRead < numwords)
    {
        ...
        if(retval > 0)
        {
            if((numbytes = recvfrom(m_socket,buffer,2*how_much_to_read,0,(struct sockaddr*)&their_addr,&addr_len)) == -1)
            {
                perror("recvfrom");
            ...
            if(numwords-wordsRead<1456)
            {
                how_much_to_read = numwords - wordsRead;
            }
        }else if(retval == 0)
        {
    ...
    for(int i_data=0;i_data<data.size();i_data+=4)
    {
        if(i_data==7796)
        {
            reversed_data.push_back(data[i_data+2]);
            reversed_data.push_back(data[i_data+1]);
            reversed_data.push_back(data[i_data+0]);
        }else
        {
            reversed_data.push_back(data[i_data+3]);
            reversed_data.push_back(data[i_data+2]);
            reversed_data.push_back(data[i_data+1]);
            reversed_data.push_back(data[i_data+0]);
        }
