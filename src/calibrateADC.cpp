...
//	system("mkdir -p autogenerated_calibrations");
	system("rm ./Results/Raw_PedScan_*.txt");
	...
	for(int iPed = 0x010; iPed < 0xfff; iPed += 0x010)
	{
	    printf("Starting ped %d\n", iPed);
	    acc.setPedestals(boardMask, 0x1f, iPed);

	    usleep(100000);
	
	    acc.dumpData(boardMask);
	
	    for(int i=0; i<N_EVENTS; i++)
	    {
		acc.softwareTrigger();

		retval = acc.listenForAcdcData(trigMode, raw, "PedScan_" + std::to_string(iPed));
		if (retval!=0)
		{
