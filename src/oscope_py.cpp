...
int main()
{
	...
	while(true)
	{
		std::cout << "What port is the ACDC connected on the ACC? If you're unsure run ./bin/connectedBoards" << std::endl;
	...
	int first=0;
	std::string basecmd = "python3 ./analysis/oscope/ ";
	std::string cmd0 = basecmd + "0 5 " + to_string(board);
	std::string cmd1 = basecmd + "6 11 " + to_string(board); 
	std::string cmd2 = basecmd + "12 17 " + to_string(board); 
	std::string cmd3 = basecmd + "18 23 " + to_string(board); 
	std::string cmd4 = basecmd + "24 29 " + to_string(board);  
	failCounter = 0;
